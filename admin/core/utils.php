...
function get_heading($heading = '', $return_pos = false)
{
    $app = get_config();
    $heading = ($heading) ? $heading : HEADING;

    foreach ($app['headings'] as $ind => $head_temp) {
        if ($heading == $head_temp['name']) {
            return ($return_pos) ? $ind : $head_temp;
        }
    }
    return array();
}
...
function trace($slug)
{
    e(heading('table', 'log'), ['content' => addslashes($slug)]);
}
