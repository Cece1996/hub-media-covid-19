<div class="row wrapper border-bottom white-bg page-heading">
    ...
    <div class="col-sm-8">
        <?php if (has_right(heading('add'))) : ?>
            <div class="title-action">
                <a href="<?= route('add') ?>" class="btn btn-primary d-print-none"><i class="fa fa-plus"></i> Ajouter <?= heading('male') ? 'un' : 'une' ?> <?= heading('singular') ?></a>
            </div>
        <?php endif; ?>
    </div>
</div>
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php foreach ($data as $item) : ?>
                                <?php if (session('p') < 4 and $item['profile'] == 'Super administrateur') continue; ?>
                                <tr>
                                    ...
                                    <td class="d-print-none">
                                        <?php foreach (heading('listing')['actions'] as $action) : ?>
                                            <?php if (($action['type'] == 'delete' and session('e') == $item['email']) or (session('p') != 99 and $action['type'] == 'delete' and $item['profile'] == 'Super administrateur')) continue; ?>
    ...
    <?php else : ?>
        <div class="middle-box text-center animated bounceIn">
            ...
            <div class="error-desc">
                ...
                <a href="<?= route('add') ?>" class="btn btn-primary m-t"><i class="fa fa-plus"></i> Ajouter <?= heading('male') ? 'un' : 'une' ?> <?= heading('singular') ?></a>
            </div>
        </div>
    <?php endif; ?>
</div>